...
void setup() { //set up, unified across the whole program //DONT NEED CONSOLE ENABLE ON THIS BECAUSE IT IS SET UP
...
void keyPressed() { //if key pressed
  ...
  if (key == 'i' && consoleEnable == 1) { //if it was i
    ...
    textplace1 = 35;
    fill(0);
    text("Press 'e' to make lines follow your mouse", 0, textplace1); //FROM HERE ON BELOW IT IS SOLELY ME EXPLAINING WHAT EACH KEY IS ASSIGNED TO, THEN A SHIFT DOWN THE LINE AND SO ON
    textplace1 = textplace1 + 15;
  ...
  if (key == 'n') { //if the key was n
    ...
    if (consoleEnable == 1) {
      ...
      println("Circlesize set to small");
    }
  }
  ...
  if (key == 'v') {
    ...
    if (colour < 7);
    if (consoleEnable == 1) {
      println("Colour changed to: " +colour);
      textplace1 = textplace1 + 15;
    }
    else if (consoleEnable == 0) {
      println("Colour changed to: " +colour);
...
void mousePressed() { //consoleEnable has been filled on this
  if (mouseButton == LEFT) {
    lineX = mouseX;
    lineY = mouseY;
    if (consoleEnable == 1) {
      text("Coordinate " +lineX + " x set.", 0, textplace1);
      textplace1 = textplace1 + 15;
    }
    else if (consoleEnable == 0) {
      println("Coordinate " +lineX + " x set.");
    }
  }
  if (mouseButton == RIGHT) {
    ...
    lineX1 = mouseX;
    if (consoleEnable == 1) {
      text("Coordinate " +lineY + " Y set.", 0, textplace1);
      textplace1 = textplace1 + 15;
    }
